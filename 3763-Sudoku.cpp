...
void init()
{
    ...
    // First 256 columns representing 16*16 positions;
    // Second 256 columns representing row*num;
    // Third 256 columns representing col*num;
    // Last 256 columns representing box*num.
    for (const char *fmt : {"%c,%c", "R%c%c", "C%c%c", "B%c%c"}) {
        for (int i{1}; i <= WIDTH; ++i) {
            for (int j{1}; j <= WIDTH; ++j) {
                std::snprintf(nodes[ncnt].d.d.name, MAX_NAME, fmt, i + 'A' - 1,
                              j + 'A' - 1);
    ...
    for (int row{0}; row < WIDTH; ++row) {
        for (int col{0}; col < WIDTH; ++col) {
            ...
            for (int d{1}; d <= WIDTH; ++d) {
                ...
                // box*digit
                add_row_node(3 * SQU_SIZE + WIDTH * (row / 4 * 4 + col / 4) + d,
                             3);
...
void print_solution(int k)
{
    for (int i{0}; i < k; ++i) {
        ...
        do {
            if (nodes[nodes[n].d.col].d.d.name[1] == ',') {
                answer[nodes[nodes[n].d.col].d.d.name[0] - 'A']
                      [nodes[nodes[n].d.col].d.d.name[2] - 'A'] =
                          nodes[nodes[nodes[n].next].d.col].d.d.name[2];
                break;
            }
            n = nodes[n].next;
        } while (n != solution[i]);
    ...
    for (int row{0}; row < WIDTH; ++row) {
        for (int col{0}; col < WIDTH; ++col) {
            std::cout << static_cast<char>(answer[row][col])
                      << (col == WIDTH - 1 ? "\n" : "");
        }
    }
}
